...
Alibaba Cloud OSS command-line tool (v2.0). Upload, download, manage, and share files on Object Storage Service.

| | |
|---|---|
| Install | See [install-toolkit.sh](../scripts/install-toolkit.sh) or download from Alibaba Cloud |
| Config | `~/.ossutilconfig` or `ossutil config` interactive setup |

### Configuration

```bash
# Interactive config (sets endpoint, access key, secret)
ossutil config

# Config file format (~/.ossutilconfig)
# [Credentials]
...
ossutil cp local-file.zip oss://bucket/path/file.zip

# Upload a directory (recursive)
ossutil cp -r ./downloads/ oss://bucket/downloads/

# Incremental upload (skip unchanged files)
ossutil cp -r -u ./downloads/ oss://bucket/downloads/

# Concurrent upload (10 threads)
ossutil cp -r -j 10 ./downloads/ oss://bucket/downloads/

# Upload with progress
ossutil cp --progress local-file.zip oss://bucket/path/file.zip
```
...
```bash
# Download a file
ossutil cp oss://bucket/path/file.zip ./local-file.zip

# Download a directory
ossutil cp -r oss://bucket/downloads/ ./local-downloads/

# Incremental download
ossutil cp -r -u oss://bucket/downloads/ ./local-downloads/
```

### Presigned URLs (Sharing)

```bash
# Generate presigned URL (default expiry)
ossutil sign oss://bucket/path/file.zip

# With custom expiry (in seconds: 43200 = 12 hours)
ossutil sign --timeout 43200 oss://bucket/path/file.zip

# 7 days
ossutil sign --timeout 604800 oss://bucket/path/file.zip
```
...
ossutil ls oss://bucket/downloads/

# Short format (paths only)
ossutil ls -s oss://bucket/

# Check storage usage (du)
ossutil du oss://bucket/
ossutil du oss://bucket/downloads/
```
...
```bash
# Delete a file
ossutil rm oss://bucket/path/file.zip

# Delete a directory (recursive)
ossutil rm -r oss://bucket/old-downloads/

# Sync (mirror local to OSS)
ossutil sync ./local-dir/ oss://bucket/remote-dir/

# Sync (mirror OSS to local)
...
```bash
# Get object metadata
ossutil stat oss://bucket/path/file.zip
...
| Upload to Alibaba Cloud OSS | **ossutil** `cp` |
| Share files via presigned URL | **ossutil** `sign` |
| Browse OSS storage | **ossutil** `ls` / `du` |
...
# ossutil (macOS, auto-detect arch)
# See scripts/install-toolkit.sh for full cross-platform installer

# Verify
yt-dlp --version && gallery-dl --version && aria2c --version && ossutil version
```
